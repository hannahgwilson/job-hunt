...
async function handleScheduleInterview(
  ...
  // (application, scheduled_at, interview_type), so a re-run calendar import is
  // a no-op instead of a second copy. Only then do we touch the calendar: this
  // ordering is what keeps a duplicate call from minting an orphan `events`
  // row, which is exactly what the old insert-event-then-insert-interview
  // sequence did. (Pre-021 this handler inserted into `interviews` directly and
  // had no dedup at all.)
